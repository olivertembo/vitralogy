...
import { SNOW_SUPPORT_DOC_FOR_LINE_ITEM } from "../../../../constants/api"
import CommonProof from "./CommonProof"

const OtherDocument = ({ item, auth, onProofSubmitted, disabled }) => {
  const { invoiceId, lineItemId } = item
  const url = SNOW_SUPPORT_DOC_FOR_LINE_ITEM(invoiceId, lineItemId)
