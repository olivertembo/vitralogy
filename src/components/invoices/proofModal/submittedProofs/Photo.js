...
import { SNOW_PHOTO_FOR_LINE_ITEM } from "../../../../constants/api"
import CommonProof from "./CommonProof"

const Photo = ({ item, auth, isPostAction, disabled, onProofSubmitted }) => {
  const { invoiceId, lineItemId } = item
  const url = `${SNOW_PHOTO_FOR_LINE_ITEM(
    invoiceId,
    lineItemId,
...
const AfterPhoto = props => Photo({ ...props, isPostAction: true })
const BeforePhoto = props => Photo({ ...props, isPostAction: false })

export { AfterPhoto, BeforePhoto }
